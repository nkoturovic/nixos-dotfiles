...
**D61 — GLM-5.2 → GLM-5.3 in-place promotion keeps stable compatibility
identity (catalog21).**
The Z.ai GLM-5.3 release supersedes GLM-5.2 on the same Qwen Token Plan
route, and the user attests their Alibaba subscription now exposes it.
Official Z.ai documentation confirms: exact model ID `glm-5.3`,
1,000,000-token context, 128K max output, text-only, always-on reasoning
with low/high/max effort (default max), Anthropic Messages API with tools
and streaming. The Alibaba Token Plan web allowlists fetched 2026-08-21
still stop at GLM-5.2 and predate the release — public listing evidence
lags, so the live route is accepted only after one separately approved
bounded call before activation.

- **In-place promotion, not a new model.** The trusted `glm52` entry is
  modified in place — wire `glm-5.2` → `glm-5.3`, display `GLM-5.2` →
  `GLM-5.3`, declared context 1,048,576 → the exact official 1,000,000,
  and current qualification/routing evidence. Catalog key `glm52`,
  selector `claude-multi-glm52-max[1m]`, gateway alias
  `claude-multi-glm52-max`, and generated IDs (`cm-reviewer-glm52-max`)
  are compatibility identities and stay. This is the Qwen3.8 Preview →
  production pattern (D50): stable identity, new wire/display/evidence.
  (Correction of an earlier assumption: GLM-5.2 did not replace GLM-5.1
  in this repository — it was the first GLM entry, so D45 remains the
  historical GLM-5.2 record, unmodified.)
- **No rename, no migration.** Renaming the key/selector would rewrite
  the 17 records/scopes and 11 live composition files that pin `glm52`;
  keeping them makes this ordinary catalog drift. Compositions and state
  are untouched; catalog20 records remain resolvable.
- **Provider contract unchanged.** Provider `qwen`, 1M client/provider
  context, 200K validated floor (docs do not move the evidence ledger —
  the separately approved live call may), `large` profile, scalar null,
  single `max` lane, `reasoning-effort-max`, `agent_effort: max`,
  alibaba independence family, roles, minimum-tested versions: all
  unchanged. No new provider, endpoint, secret, or payload contract.
- **General-tier parity without roster churn.** GLM-5.3 is positioned on par
  with Qwen3.8 Max and GPT-5.6 Sol. GLM and Qwen already share identical role
  hints; existing composition order is not rewritten because explicit
  `preferred` flags—not alternate display order—remain the routing authority.
  The catalog notes choose by role/route fit rather than preserving D50's old
  Qwen-ahead-of-GLM-5.2 hierarchy. Historical D50 text remains historical.
- **Catalog-only release.** catalog 20 → 21; launcher stays 2.20.0 (no
  launcher/renderer/adapter behavior change). Only the two intended
  route lines move in the render golden; every compiler/scope golden
  stays byte-identical.
- **Evidence split and honest boundary.** The source migration is built
  from official Z.ai contract facts plus user-attested route
  availability; the stale public allowlist is recorded as the caveat it
  is. Live acceptance (one bounded Alibaba call: `glm-5.3`,
  `reasoning_effort: max`, streaming, one marker tool, no forced
  `tool_choice`, ~512 max tokens) remains a separate approval gate and
  must pass before activation; any failure blocks activation and must
  not fall back to GLM-5.2.
- **Zero-live cutover gate.** Before activation, a metadata-only census must
  find no live/mid-turn ordinary GLM session, GLM lead, or managed session with
  an enabled GLM variant. Any hit aborts activation until the process exits or
  is stopped safely. This prevents the stable alias from switching models
  inside a running conversation; `doctor --repair-all` follows only after the
  gate and local route checks pass.
- **Rollback is semantically sensitive.** Pre-activation, generation 130
  restores GLM-5.2 and no composition restoration is needed. After any
  GLM-5.3 request has been served, stable identity `glm52` would map
  back to GLM-5.2 — stop GLM-bearing sessions and prefer fixing forward.
  Records, scopes, credentials, compositions, and transcripts are never
  deleted or rewritten.
- **Tests.** Catalog pins the stable identity, exact GLM-5.3 wire/display,
  exact 1M context values, 200K floor, unchanged max contract, and the
  qualification boundary; render pins alias stability and rejects a
  rendered glm-5.2; the disposable loopback Qwen route proves bearer
  auth, alias → exact `glm-5.3` wire, max effort, offered-tool
  preservation, no invented `tool_choice`, and D60 retention stripping
  without contacting Alibaba; compiler/context pin wire resolution and
  the 1M/882K lead policy with the large-profile selector tuple
  unchanged; CLI presents GLM-5.3 while ordinary model id `glm52`,
  session name `cg:glm52`, and the selector stay.
